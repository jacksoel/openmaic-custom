...
import {
  Key, Plus, Trash2, Eye, EyeOff, CheckCircle2, XCircle,
  Loader2, ArrowLeft, Shield, Server, User, AlertTriangle,
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to add provider'); }
  ...
  const handleUpdate = async (slug: string) => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to update provider'); }
  ...
  const handleDelete = async (slug: string) => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to delete provider'); }
      ...
      <div className="max-w-3xl mx-auto px-6 py-6 space-y-6">
        <div className="rounded-lg border bg-muted/30 p-4">
          <div className="flex gap-3">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              <p>Your API keys are <strong>encrypted</strong> on the server and never sent to the browser. When you provide your own key, the system uses it instead of the institutional default.</p>
        ...
        <div className="space-y-3">
          ...
          {providers.length === 0 && (
            <div className="rounded-lg border border-dashed p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">No providers configured yet. Add your own API keys below.</p>
              <p className="text-xs text-muted-foreground/70 mt-1">Without custom keys, the system uses institutional defaults.</p>
            </div>
          )}
            ...
            <div key={provider.providerSlug} className="rounded-lg border bg-card overflow-hidden">
              <div className="px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => handleTest(provider.providerSlug)} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Test connection">
                    {testStatus[provider.providerSlug] === 'testing' ? <Loader2 className="h-4 w-4 animate-spin" />
                      : testStatus[provider.providerSlug] === 'success' ? <CheckCircle2 className="h-4 w-4 text-green-500" />
                      : testStatus[provider.providerSlug] === 'error' ? <XCircle className="h-4 w-4 text-red-500" />
                      : <Server className="h-4 w-4" />}
                  </button>
                  <button onClick={() => { setEditingSlug(editingSlug === provider.providerSlug ? null : provider.providerSlug); setEditKey(''); setEditModel(provider.defaultModel || ''); setTestStatus(prev => ({ ...prev, [provider.providerSlug]: 'idle' })); }}
                    className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Edit provider">
                    <User className="h-4 w-4" />
                  </button>
                  <button onClick={() => setDeleteSlug(provider.providerSlug)} className="p-1.5 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors" title="Remove provider">
                ...
                <div className="border-t px-4 py-3 bg-muted/20 space-y-3">
                  <div>
                    ...
                    <div className="relative mt-1">
                      <input type={visibleKeys.has(provider.providerSlug) ? 'text' : 'password'} value={editKey} onChange={e => setEditKey(e.target.value)} placeholder="sk-..."
                        className="w-full rounded-md border bg-background px-3 py-2 text-sm pr-10" />
                      <button type="button" onClick={() => setVisibleKeys(prev => { const n = new Set(prev); n.has(provider.providerSlug) ? n.delete(provider.providerSlug) : n.add(provider.providerSlug); return n; })}
                        className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                        {visibleKeys.has(provider.providerSlug) ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      </button>
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <button onClick={() => handleUpdate(provider.providerSlug)} disabled={editSubmitting} className="px-3 py-1.5 text-sm rounded-md bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors">
        ...
        <div className="space-y-3">
          ...
          {availableProviders.length === 0 && providers.length > 0 && (
            <p className="text-sm text-muted-foreground">All available providers are configured. Edit existing ones above to change keys.</p>
          )}
          {availableProviders.length > 0 && !showAdd && (
            <button onClick={() => setShowAdd(true)} className="w-full rounded-lg border border-dashed p-4 flex items-center justify-center gap-2 text-muted-foreground hover:text-foreground hover:border-primary/50 transition-colors">
              <Plus className="h-5 w-5" /><span className="text-sm font-medium">Add AI Provider</span>
            </button>
          )}
          {showAdd && (
            <div className="rounded-lg border bg-card p-4 space-y-4">
              <div>
                ...
                <select value={addSlug} onChange={e => { setAddSlug(e.target.value); setAddModel(''); }} className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm">
                  <option value="">Select a provider...</option>
                  {availableProviders.map(p => <option key={p.slug} value={p.slug}>{p.name}</option>)}
                </select>
              </div>
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <button type="button" onClick={() => setVisibleKeys(prev => { const n = new Set(prev); n.has('new') ? n.delete('new') : n.add('new'); return n; })} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    {visibleKeys.has('new') ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </button>
                </div>
                <p className="text-xs text-muted-foreground mt-1">Your key is encrypted on the server and never exposed to the browser.</p>
              </div>
              {addSlug && getProviderModels(addSlug).length > 0 && (
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Default Model (optional)</label>
                  <select value={addModel} onChange={e => setAddModel(e.target.value)} className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm">
                    <option value="">Use provider default</option>
                    {getProviderModels(addSlug).map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                </div>
              )}
              <div className="flex gap-2 justify-end pt-2 border-t">
                ...
                <button onClick={handleAdd} disabled={!addSlug || !addKey || addSubmitting} className="px-4 py-2 text-sm rounded-md bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors flex items-center gap-2">
                  {addSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" /><span>Saving...</span></> : <><Key className="h-4 w-4" /><span>Add Provider</span></>}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="rounded-lg border p-4">
          <div className="flex gap-3">
            ...
            <div>
              <h3 className="text-sm font-medium">Institutional Defaults</h3>
              <p className="text-xs text-muted-foreground mt-1">When you don't configure your own provider, the system uses institutional keys. These are managed by the admin and available to all users. Your own keys take priority when configured.</p>
            </div>
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="rounded-lg border bg-card p-6 max-w-sm w-full mx-4 shadow-xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div><h3 className="font-medium">Remove Provider</h3><p className="text-sm text-muted-foreground">{getProviderName(deleteSlug)}</p></div>
              </div>
              <p className="text-sm text-muted-foreground mb-4">This will delete your API key for this provider. The system will fall back to institutional defaults. This action cannot be undone.</p>
              <div className="flex gap-2 justify-end">
                ...
                <button onClick={() => handleDelete(deleteSlug)} disabled={deleteSubmitting} className="px-3 py-1.5 text-sm rounded-md bg-destructive text-destructive-foreground hover:bg-destructive/90 disabled:opacity-50 transition-colors">
