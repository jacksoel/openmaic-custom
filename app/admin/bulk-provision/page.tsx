...
export default function BulkProvisionPage() {
  ...
  async function handleSubmit() {
    ...
    if (validRows.length === 0) { setSubmitError('No valid rows to submit.'); return; }
    setSubmitting(true);
    setSubmitError(null);
    setResult(null);
    try {
      const res = await fetch('/api/admin/bulk-provision', {
        ...
        body: JSON.stringify({ users: validRows.map(r => ({ email: r.email, name: r.name, role: r.role, ...(r.password ? { password: r.password } : {}) })) }),
      });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <button onClick={() => router.push('/admin')} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
              &larr; Back to Admin
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Columns: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">email, name, role, password</code>{' '}
            (role and password are optional). First row may be a header.
          </p>
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Default Role (used when role column is empty)</label>
          ...
          <textarea value={csvText} onChange={e => setCsvText(e.target.value)}
            ...
            rows={8}
            className="w-full text-sm font-mono border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 resize-y"
          />
          <div className="mt-4 flex gap-3">
            <button onClick={handleParse} disabled={!csvText.trim()}
              className="px-4 py-2 text-sm rounded-lg bg-gray-800 dark:bg-gray-600 text-white hover:bg-gray-700 dark:hover:bg-gray-500 disabled:opacity-50 transition-colors">
              Parse &amp; Preview
            </button>
          ...
          {submitError && (
            <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-700 dark:text-red-400 text-sm">{submitError}</div>
          )}
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">
                ...
                {errorCount > 0 && <span className="ml-2 text-sm text-red-600 dark:text-red-400 font-normal">({errorCount} with errors)</span>}
              </h2>
            </div>
                    ...
                    <tr key={i} className={row.errors.length > 0 ? 'bg-red-50 dark:bg-red-900/10' : ''}>
                      ...
                      <td className="px-4 py-3">
                        {row.errors.length === 0
                          ...
                          : <span className="text-red-600 dark:text-red-400 text-xs">{row.errors.join('; ')}</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
                    ...
                    <div key={i} className="px-6 py-3 flex items-center justify-between">
                      ...
                      {u.tempPassword && (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => copyToClipboard(u.tempPassword!, i)} className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" title="Copy password">
            ...
            {result.skipped.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-yellow-50 dark:bg-yellow-900/20">
                  <h2 className="text-lg font-semibold text-yellow-800 dark:text-yellow-300">Skipped ({result.skipped.length})</h2>
                </div>
                <div className="divide-y divide-gray-100 dark:divide-gray-700">
                  {result.skipped.map((u, i) => (
                    <div key={i} className="px-6 py-3 flex items-center justify-between">
                      <span className="text-sm text-gray-900 dark:text-gray-100">{u.email}</span>
                      <span className="text-xs text-yellow-700 dark:text-yellow-400">{u.reason}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {result.errors.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-red-50 dark:bg-red-900/20">
                  <h2 className="text-lg font-semibold text-red-800 dark:text-red-300">Errors ({result.errors.length})</h2>
                </div>
                <div className="divide-y divide-gray-100 dark:divide-gray-700">
                  {result.errors.map((u, i) => (
                    <div key={i} className="px-6 py-3 flex items-center justify-between">
                      <span className="text-sm text-gray-900 dark:text-gray-100">{u.email}</span>
                      <span className="text-xs text-red-600 dark:text-red-400">{u.error}</span>
                    </div>
                  ))}
                </div>
              </div>
