...
export default function AdminPage() {
  ...
  async function updateRole(userId: string, newRole: string) {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to update role'); }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/settings')} className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 flex items-center gap-1"><Key className="w-3.5 h-3.5" />My AI Stack</button>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage user roles. Admins and instructors can create classrooms and configure AI providers.</p>
          </div>
            ...
            {users.map(user => (
              <div key={user.id} className="px-6 py-4 flex items-center justify-between">
                ...
                <div className="ml-4 flex items-center gap-2">
                  ...
                  {updating === user.id && <span className="text-xs text-gray-400 dark:text-gray-500">Updating...</span>}
                </div>
              </div>
            ))}
          </div>
          {users.length === 0 && <div className="px-6 py-12 text-center text-gray-400 dark:text-gray-500">No users found.</div>}
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="rounded-xl border bg-white dark:bg-gray-800 dark:border-gray-700 p-6 max-w-sm w-full mx-4 shadow-xl">
              ...
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                Change <strong>{target?.name || target?.email}</strong> from <strong>{target?.role}</strong> to <strong>{pendingRoleChange.newRole}</strong>?
              </p>
              <div className="mt-5 flex gap-2 justify-end">
                <button onClick={() => setPendingRoleChange(null)} className="px-4 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors dark:text-gray-200">Cancel</button>
                <button onClick={() => { updateRole(pendingRoleChange.userId, pendingRoleChange.newRole); setPendingRoleChange(null); }} className="px-4 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors">Confirm</button>
              </div>
            </div>
          </div>
        );
