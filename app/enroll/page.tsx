...
function ClassroomLookupSkeleton() {
  return (
    <div className="space-y-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 p-4" aria-label="Loading classroom preview">
...
function EnrollForm() {
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(res.status === 404 ? "Classroom not found. Check the code and try again." : "Unable to look up classroom. Please try again.");
        return;
      }
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.detail || (res.status === 403 ? "You do not have permission to enroll in this classroom." : "Enrollment failed. Please try again.")); return; }
      if (data.enrolled === false) { setEnrolledClassId(preview.id); setToastMessage("You are already enrolled in this classroom. Open it from your dashboard or jump straight in."); return; }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 px-4 py-8">
      ...
      <div className="mx-auto grid w-full max-w-4xl gap-6 lg:grid-cols-[1fr_1.2fr]">
        <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-6 rounded-lg border border-blue-100 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-4 text-sm text-blue-800 dark:text-blue-300">
        ...
        <section className="relative w-full space-y-6 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm sm:p-8">
          {toastMessage && (
            <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-3 text-sm text-amber-800 dark:text-amber-300" role="status" aria-live="polite">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <span>{toastMessage}</span>
                <div className="flex items-center gap-2 shrink-0">
                  <button type="button" onClick={() => router.push(enrolledClassId ? `/classroom/${enrolledClassId}` : "/dashboard")}
                    className="rounded-md bg-amber-100 dark:bg-amber-800 px-3 py-1.5 text-xs font-medium text-amber-900 dark:text-amber-200 hover:bg-amber-200 dark:hover:bg-amber-700">Open classroom</button>
                  <button type="button" onClick={() => { setToastMessage(null); setEnrolledClassId(""); }}
                    className="rounded-md px-2 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/30 leading-none" aria-label="Dismiss">×</button>
                </div>
              </div>
            </div>
          )}
          ...
          {enrolled ? (
            <div className="space-y-4 text-center">
              <div className="rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-5">
                <p className="font-medium text-green-700 dark:text-green-300">Enrolled in {preview?.name || 'classroom'}! Redirecting to dashboard...</p>
                <p className="mt-1 text-sm text-green-600 dark:text-green-400">Redirecting you to your dashboard in 2 seconds.</p>
              </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleLookup(); }} className="space-y-5">
              {error && (
                <div className="flex items-start gap-2 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 p-3 text-sm text-red-700 dark:text-red-400" role="alert">
                  <span>{error}</span>
                </div>
              )}
              <div className="space-y-2">
                <label htmlFor="classroomCode" className="text-sm font-medium text-gray-700 dark:text-gray-300">Classroom Code</label>
                <input id="classroomCode" type="text" value={classroomCode} onChange={(e) => { setClassroomCode(e.target.value); resetMessages(); }}
                  required autoComplete="off" disabled={loading} placeholder="e.g. my-classroom-2024"
              ...
              <button type="submit" disabled={loading || !classroomCode.trim()}
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <button onClick={() => { setPreview(null); resetMessages(); }} disabled={loading}
                  className="rounded-lg border border-gray-200 dark:border-gray-700 px-4 py-2.5 text-sm font-medium transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 dark:text-gray-200">Back</button>
